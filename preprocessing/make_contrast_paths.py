...
search_directory = "/path/to/HCP"
output_directory = os.path.abspath("../")
doc_directory = "%s/doc" %(output_directory)
copes_directory = "%s/data/copes" %(output_directory)

if not os.path.exists(doc_directory):
    os.mkdirs(doc_directory)
...
# This file was manually filtered to remove redundant contrasts (hcp_contrasts_filt)
...
for contrast in all_contrasts.iterrows():
  ...
  output_file = "%s/copes/%s_%s_copes.txt" %(copes_directory,contrast[1].task,contrast[1].contrasts)
...
# STEP 3: RAW TIMESERIES DATA LISTS -------------------------------------------------
# First we will find all the volume feat directories
timeseries_files = []
for root, dirnames, filenames in os.walk(search_directory):
